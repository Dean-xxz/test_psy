...
from utils.view_tools import ok_json
APP_PAGINATOR_DEFAULT_WIDTH = 5
...
class CommonPaginator(BasePaginator):
    ...
    def __init__(self, result, wrapper, page, page_size, request, limit=None, \
            pager_width=APP_PAGINATOR_DEFAULT_WIDTH, \
            pager_lmargin=APP_PAGINATOR_DEFAULT_LMARGIN, **params):
...
class PartPaginator(BasePaginator):

    def __init__(self, total_count, result, wrapper, page, page_size, request, limit=None, \
            pager_width=APP_PAGINATOR_DEFAULT_WIDTH, \
            pager_lmargin=APP_PAGINATOR_DEFAULT_LMARGIN, **params):
    ...
    def entries(self):
        if self.wrapper:
            self._entries = self.wrapper(self.result)
        else:
            self._entries = self.result
